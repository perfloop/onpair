...
func TestModelSharedSuffixKeyFixture(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "large", variants: 273},
		{name: "default-cap", variants: 8192},
		{name: "configured-cap", variants: 16384, sampleBytes: 2 * 1024 * 1024},
	} {
		t.Run(tc.name, func(t *testing.T) {
			model, rows := trainModelSharedKeyFixture(t, tc.variants, true, tc.sampleBytes)
			...
			t.Logf("variants=%d previous-chain-visits=%d group-slot-probes=%d", tc.variants, visits, probes)
		})
	}
}
...
const (
	longTailRejectPrefix    = "abcdefgh"
	longTailRejectLengthGap = 64
	longTailRejectMaxLength = longTailRejectLengthGap * 64
)
...
func newLongTailRejectFixture(tb testing.TB, variants, tailLen int) longTailRejectFixture {
	...
	for i := 0; i < variants; i++ {
		// Spread the 64 populated lengths from 64 through 4,096 bytes so a
		// long rejecting tail hashes every selected prefix length, not merely
		// empty bitmap words above a short candidate set.
		suffixLen := longTailRejectLengthGap * (1 + i%64)
	...
	if maxSuffixLen > longTailRejectMaxLength {
		maxSuffixLen = longTailRejectMaxLength
	}
	probes, visits, suffixKeyBytes := 0, 0, 0
	for suffixLen := maxSuffixLen; suffixLen >= longTailRejectLengthGap; suffixLen -= longTailRejectLengthGap {
		suffixKeyBytes += suffixLen
	...
	return longTailRejectFixture{
		...
		lengthGroups:    maxSuffixLen / longTailRejectLengthGap,
		groupSlotProbes: probes,
		suffixKeyBytes:  suffixKeyBytes,
		chainVisits:     visits,
	}
}
...
func TestLongTailRejectFixture(t *testing.T) {
	for _, tailLen := range []int{64, 1024, 4096} {
		...
		if fixture.chainVisits != 0 || fixture.groupSlotProbes < fixture.lengthGroups || fixture.suffixKeyBytes == 0 {
			t.Fatalf("tail %d: chain visits %d, group-slot probes %d, suffix-key bytes %d", tailLen, fixture.chainVisits, fixture.groupSlotProbes, fixture.suffixKeyBytes)
		}
	}
}
...
func benchmarkModelEncodeLongTailReject(b *testing.B, tailLen int) {
	...
	b.ReportMetric(float64(fixture.lengthGroups), "suffix-key-hashes/row")
	b.ReportMetric(float64(fixture.suffixKeyBytes), "suffix-key-bytes/row")
